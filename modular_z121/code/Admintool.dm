...
/obj/item/vi_katana
	...
	var/datum/action/innate/dash/vi/jaunt

/obj/item/vi_katana/Initialize()
	. = ..()
	jaunt = new(src)
...
/obj/item/vi_katana/afterattack(atom/target, mob/user, proximity_flag, click_parameters)
	. = ..()
...
/*
/obj/item/vi_katana/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	if(ishuman(hit_atom))
 ...
 */
/obj/item/vi_katana/proc/returnToOwner(mob/living/carbon/human/user, doSpark = TRUE, caught = FALSE)
	if(!istype(user))
		return
...
/datum/action/innate/dash/vi
	current_charges = 10000
	max_charges = 10000
	charge_rate = 200
	recharge_sound = null


/obj/machinery/power/rtg/super
	power_gen = 100000
...
/obj/item/vi_katana
	...
	resistance_flags = LAVA_PROOF | FIRE_PROOF | ACID_PROOF
	var/datum/effect_system/spark_spread/spark_system
	var/datum/action/innate/dash/ninja/vi/jaunt

/obj/item/vi_katana/Initialize(mapload)
	. = ..()
	jaunt = new(src)
	spark_system = new /datum/effect_system/spark_spread()
	spark_system.set_up(5, 0, src)
...
/obj/item/vi_katana/afterattack_secondary(atom/target, mob/user, proximity_flag, click_parameters)
	. = ..()
...
/obj/item/vi_katana/equipped(mob/user, slot, initial)
	. = ..()
	if(!QDELETED(jaunt))
		jaunt.Grant(user, src)
...
/datum/action/innate/dash/ninja/vi
	current_charges = 9999
	max_charges = 9999
	charge_rate = 350 //SKYRAT EDIT CHANGE - ORIGINAL: 200
	beam_length = 1 SECONDS
	recharge_sound = null

/datum/action/innate/dash/ninja/vi/GiveAction(mob/viewer) //this action should be invisible, as its handled by right-click
	return

/datum/action/innate/dash/ninja/vi/HideFrom(mob/viewer)
	return
*/