	...
	switch(stage)
		...
		if(2)
			if(SPT_PROB(0.8, seconds_per_tick)) // NOVA EDIT CHANGE - ORIGINAL: if(SPT_PROB(0.5, seconds_per_tick))
				to_chat(affected_mob, span_warning(LANG("datum.32fc3956", null)))
				affected_mob.adjust_confusion(6 SECONDS)
			if(SPT_PROB(0.8, seconds_per_tick)) // NOVA EDIT CHANGE - ORIGINAL: if(SPT_PROB(0.5, seconds_per_tick))
				to_chat(affected_mob, span_notice(LANG("datum.dc61f904", null)))
				affected_mob.set_eye_blur_if_lower(10 SECONDS)
		if(3)
			var/need_mob_update = FALSE
			if(SPT_PROB(0.8, seconds_per_tick)) // NOVA EDIT CHANGE - ORIGINAL: if(SPT_PROB(0.5, seconds_per_tick))
				to_chat(affected_mob, span_danger(LANG("datum.84d02cd6", null)))
				if(prob(45))
			...
			if(SPT_PROB(1, seconds_per_tick)) // NOVA EDIT CHANGE - ORIGINAL: if(SPT_PROB(0.5, seconds_per_tick))
				to_chat(affected_mob, span_danger(LANG("datum.3b9ee680", null)))
		...
		if(4)
			...
			if(SPT_PROB(1, seconds_per_tick))
				to_chat(affected_mob, span_danger(LANG("datum.da20c9d1", null)))
				if(prob(75))
			...
			if(SPT_PROB(1, seconds_per_tick))
				...
				affected_mob.visible_message(span_warning(LANG("datum.b250dd93", list(affected_mob))))
				if(prob(30))
					to_chat(affected_mob, span_danger(LANG("datum.85450d3d", null)))
		...
		if(5)
			switch(rand(1,5)) // NOVA EDIT CHANGE - ORIGINAL: switch(rand(1,2))
				if(1)
					to_chat(affected_mob, span_notice(LANG("datum.7072c9ee", null)))
					update_stage(1)
				if(2)
					to_chat(affected_mob, span_boldwarning(LANG("datum.1a15cd3b", null)))
					...
					affected_mob.visible_message(span_warning(LANG("datum.a91a8b1c", list(affected_mob))), span_userdanger(LANG("datum.c9102b9e", null)))
				...
				if(3)
					to_chat(affected_mob, span_warning(LANG("datum.9c356b27", null)))
					affected_mob.visible_message(span_warning(LANG("datum.70a35d13", list(affected_mob))), span_userdanger(LANG("datum.5387ba7e", null)))
					...
					update_stage(1)
				if(4)
					affected_mob.visible_message(span_warning(LANG("datum.255b4f2b", list(affected_mob))), span_userdanger(LANG("datum.e983531f", null)))
